...
## ファイルの説明
- **JAIRO_Cloud_log_parser_YYYYMMDD.xlsx**（YYYYMMDD はリリース日。現在の配布ファイルは `JAIRO_Cloud_log_parser_20260715.xlsx`）
...
![［統計］＞［運用レポート］](https://jpcoar.org/system/wp-content/uploads/2026/02/jc_stat10.png)
...
- シート Filedownload (OA)
  ...
  - 出力されたファイルのうち、logReport_FileDownload_{年}-{月}.tsvを集計します。
  - このシートは、ファイルの `accessrole` が `open_access` に設定されているダウンロードのみを集計します。公開日を過ぎた `open_date` は含まれません。全ダウンロード（File Download シート）の部分集合です。
- シート Detail View
  - 定型レポートのうち各月の「アイテムビュー」を結合したデータです。
  - 出力されたファイルのうち、logReport_DetailView_{年}-{月}.tsvを集計します。
- シート OA Difference
  - シート File Download（全ダウンロード）とシート Filedownload (OA)（OA ダウンロード）を、年月・File Name・Registered Index Name で突合し、差分を確認するためのデータです。
  - 各行の Status は、All Downloads と OA Downloads の関係で「OA一致」（一致）／「OA側なし」（OA 側に該当なし）／「部分一致」（OA 側が一部）／「要確認」（OA が全体を超えるなどの不整合）に分類されます。
  - Difference（All Downloads − OA Downloads）の要因には、`accessrole` が `open_date`／`open_login`／`open_no`／未設定のファイル、旧データ移行時の設定差、元レポートの不整合などが考えられます。このシートは `accessrole` を取得しないため原因を断定するものではなく、確認には item の JSON（`accessrole` 等）を参照してください。
...
- [用語集](docs/06_glossary.md)
  - アイテム・ファイル・`accessrole`・OAダウンロードなど、統計と本ツールの理解に必要な用語をまとめています。
...
## 更新
- 2026/07/15
  - 統計値の意味・集計値の選択・単純加算の注意を README に追記
  - シート Filedownload (OA) に accessrole=open_access 限定の説明を追加
  - 全ダウンロードと OA ダウンロードの差分を確認するシート OA Difference を追加
  - Usage and Config に目的別の統計値選択ガイドを追加
  - 動作確認用の合成データと期待結果（samples/）を追加
- 2025/01/04
  - 最初のリリース
