...
一般には、誰でも利用できる公開状態を指します。ただし、JAIRO Cloud の「OAダウンロード統計」は、ファイルの `accessrole` が `open_access` であるダウンロードだけを対象とする技術的な区分です。現在誰でも利用可能なファイルすべてを表すものではありません。
...
以下は、保存されている設定値と実際の利用可否を区別するため、本用語集で使用する説明上の表現です。JAIRO Cloud の内部フィールド名や定型レポートの列名ではありません。
...
## ファイルの公開条件（`accessrole`）

各ファイルの公開条件は、内部フィールド `accessrole` に保存されます。`accessrole` は設定値であり、その時点で実際に誰がファイルを取得できるかを、この値だけで直接表すものではありません。

ファイルのダウンロード可否を判定する際には、`accessrole` を主要な条件として、アイテム公開状態・利用可能日・ログイン状態・利用者の権限なども参照されます。また、統計イベントにはアクセス時に再判定した値ではなく、ファイルの JSON に保存された `accessrole` が**そのまま**記録されます。OA ダウンロード統計は、その値が `open_access` であるイベントだけを対象とします。

### アクセス権（`accessrole`）の種類

- **`open_access`**
  - ファイルが原則として誰でも利用可能な状態を表します。OA ダウンロード統計の対象は、この値のファイルだけです。
- **`open_date`**
  - 指定日以降に公開するファイルを表します。
  - **公開日を過ぎても値は `open_date` のまま**で、`open_access` へは自動的に変更されません。そのため、公開日を過ぎて誰でもダウンロードできる状態でも、OA 統計には含まれません。
- **`open_login`**
  - ログイン利用者等に限定して公開するファイルを表します。
- **`open_no`**
  - 非公開のファイルを表します。
- **`open_restricted`**
  - 制限公開機能による条件付き公開のファイルを表します。
- **`accessrole` 未設定・空欄**
  - 明示的な公開区分が保存されていない状態を表します。移行データ等で生じることがあります。
...
- **JSON**
  - データを項目名と値の組み合わせで記述するテキスト形式。本ツールが参照する JAIRO Cloud のアイテムやファイルの設定情報にも使われ、ファイルの `accessrole` もここに保存されます。
- **行レベルデータ**
