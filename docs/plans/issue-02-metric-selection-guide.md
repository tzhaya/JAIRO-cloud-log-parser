...
1. README に「集計値の選択」セクションを新設し、docs/04 の Issue 2 提案文をそのまま採用する：
   ...
   - ダウンロードは `File Download` シート、詳細閲覧は `Detail View` シートに、それぞれ行レベルの元データを保持していること。両者は別統計であり、それぞれの用途に応じて機関独自の基準で再集計できること（詳細閲覧データからダウンロードを再集計できるという意味ではない。実ワークブックのシート名は `File Download`／`Detail View` でスペースあり）
